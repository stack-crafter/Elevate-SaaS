  ...
  head: () => ({
    meta: [
      { title: "Elevate" },
      {
        name: "description",
        content:
          "The calm, precise assessment platform used by Fortune 500s, universities, and recruiters.",
      },
      { property: "og:title", content: "Elevate Assess" },
      {
        property: "og:description",
        content: "Stripe for technical assessments, reimagined in daylight.",
      },
    ],
  }),
  component: Landing,
    ...
    <div className="relative min-h-screen bg-background text-foreground">
      <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display text-[15px] font-bold tracking-tight">Elevate</span>
        </div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-6 pb-24 pt-16 text-center md:pt-24">
          <div className="mx-auto mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-white/70 px-3 py-1.5 text-xs font-medium text-muted-foreground backdrop-blur-sm">
            <span className="h-1.5 w-1.5 rounded-full bg-primary" /> Trusted by 400+ engineering
            teams
          </div>
          <h1 className="font-display text-5xl font-extrabold leading-[1.05] tracking-tight md:text-7xl">
            Technical assessments,{" "}
            <span className="relative inline-block">
              <span className="bg-gradient-to-r from-[#6b7280] via-[#4b5563] to-[#1f2937] bg-clip-text text-transparent">
                reimagined in daylight.
              </span>
            </span>
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground md:text-xl">
            The calm, AI-native platform Fortune 500s, universities, and recruiters use to evaluate
            real engineering skill — with precision and warmth.
          </p>
            ...
            <Spotlight warm className="surface-card relative rounded-2xl p-2">
              ...
              <div className="relative overflow-hidden rounded-xl border border-border bg-surface-1">
                <div className="flex items-center justify-between border-b border-border bg-white/60 px-4 py-2.5">
                  ...
                  <div className="rounded-md border border-border bg-white px-2.5 py-1 font-mono text-[11px] text-muted-foreground">
                    elevate.app / test / python — pure code
                  </div>
                  <div className="w-14" />
                </div>
                <div className="grid gap-0 md:grid-cols-2">
                  <div className="p-6 text-left">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      ...
                      so it scales to 1M rows without external memory.
                    </p>
                    <div className="mt-4 space-y-2">
                      {[
                        "Passes: 12/12 hidden tests",
                        "Complexity: O(n log n)",
                        "Memory: within limits",
                      ].map((t) => (
                        <div
                          key={t}
                          className="flex items-center gap-2 rounded-lg border border-border bg-white px-3 py-2 text-[13px]"
                        >
                  ...
                  <div className="border-l border-border bg-[#fbfaf7] p-4 text-left font-mono text-[12.5px] leading-6">
                    <pre className="whitespace-pre text-foreground/90">
                      ...
                      <span className="text-[#7c3aed]">def</span>{" "}
                      <span className="text-[#4285f4]">find_duplicates</span>(rows):
                      <span className="text-muted-foreground">2</span> seen ={" "}
                      <span className="text-[#0f9d58]">set</span>()
                      <span className="text-muted-foreground">3</span> dupes = []
                      <span className="text-muted-foreground">4</span>{" "}
                      <span className="text-[#7c3aed]">for</span> r{" "}
                      <span className="text-[#7c3aed]">in</span> rows:
                      <span className="text-muted-foreground">5</span> key = r.id
                      <span className="text-muted-foreground">6</span>{" "}
                      <span className="text-[#7c3aed]">if</span> key{" "}
                      <span className="text-[#7c3aed]">in</span> seen:
                      ...
                      <span className="text-[#7c3aed]">return</span> dupes
                    </pre>
                  </div>
                </div>
              </div>
          ...
          <div className="relative mt-6 overflow-hidden">
            ...
            <div className="flex animate-[marquee_28s_linear_infinite] gap-16 opacity-60 [--gap:4rem]">
              {[
                ...
                <div
                  key={i}
                  className="font-display whitespace-nowrap text-xl font-bold tracking-tight text-foreground/70"
                >
                  {n}
                </div>
              ))}
      ...
      <section id="features" className="mx-auto max-w-6xl px-6 py-24">
        <div className="mb-12 max-w-2xl">
          ...
          <p className="mt-4 text-lg text-muted-foreground">
            Every surface tuned for calm focus. Every signal tuned for real engineering skill.
          </p>
        </div>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:grid-rows-2">
          <BentoCard className="md:col-span-2 md:row-span-1" ai>
            ...
            <h3 className="mt-2 font-display text-2xl font-bold">
              Adaptive questions, humane pacing.
            </h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Our model shapes each session to the candidate's signal — never wastes their time,
              never over-tests.
            </p>
          ...
          <BentoCard>
            ...
            <div className="mt-4 rounded-md border border-border bg-[#fbfaf7] p-3 font-mono text-[11.5px] leading-5">
              <div>
                <span className="text-[#7c3aed]">const</span> score ={" "}
                <span className="text-[#4285f4]">evaluate</span>(sub);
              </div>
              <div>
                <span className="text-[#7c3aed]">if</span> (score &gt;{" "}
                <span className="text-[#0f9d58]">85</span>) grant(
                <span className="text-[#4b5563]">"gold"</span>);
              </div>
            </div>
          </BentoCard>
          ...
          <BentoCard ai className="md:col-span-2">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Ship it to compliance without a fight. RBAC, SCIM, and full data residency.
            </p>
            <div className="mt-6 flex flex-wrap gap-2">
              {[
                { i: ShieldCheck, t: "SOC 2 Type II" },
                { i: Building2, t: "SSO / SAML" },
                { i: Users, t: "SCIM" },
                { i: Zap, t: "99.99% SLA" },
                { i: GraduationCap, t: "EDU tier" },
              ].map(({ i: Icon, t }) => (
                <span
                  key={t}
                  className="inline-flex items-center gap-1.5 rounded-full border border-border bg-white px-3 py-1.5 text-xs font-medium"
                >
            ...
            {[
              {
                ...
                price: "$0",
                tag: "For individuals",
                features: ["3 assessments / month", "Community support", "Public badges"],
              },
              {
                name: "Team",
                price: "$49",
              ...
              },
              {
                name: "Enterprise",
                price: "Custom",
                tag: "For 500+ engineers",
                features: ["SSO / SCIM", "SOC 2 report", "Dedicated CSM", "Custom SLAs"],
              },
            ].map((p) => (
              <div
                key={p.name}
                className={`surface-card card-hover relative p-7 ${p.recommended ? "border-primary/30" : ""}`}
              >
                {p.recommended && <BorderBeam duration={6} />}
                <div className="relative">
                  {p.recommended && (
                    <div className="absolute -top-3 right-0 rounded-full bg-primary px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-primary-foreground">
                      Recommended
                    </div>
                  )}
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-6 py-8 text-sm text-muted-foreground md:flex-row">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display font-bold text-foreground">Elevate</span>
          </div>
...
function MiniRadar() {
  ...
  return (
    <svg viewBox="0 0 120 120" className="mx-auto mt-4 h-32 w-32">
      ...
      {coords.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r="2.5" fill="#4b5563" />
      ))}
    </svg>
  );
}
