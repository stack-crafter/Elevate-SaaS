...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [{ title: "Dashboard — Elevate" }, { name: "robots", content: "noindex" }],
  }),
  component: Dashboard,
});
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 font-display text-3xl font-bold tracking-tight md:text-4xl">
              {user?.name ?? "Candidate"}, ready for another round?
            </h1>
            <p className="mt-2 text-muted-foreground">
              Ten questions. About twelve minutes. Real signal.
            </p>
          </div>
          <Link to="/select" onClick={reset}>
            <MagneticButton>
              <Sparkles className="h-4 w-4" /> New assessment <ArrowRight className="h-4 w-4" />
            </MagneticButton>
          </Link>
        </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-4">
          ...
          <Metric
            icon={Trophy}
            label="Best badge"
            value={stats.currentBadge === "None" ? "—" : stats.currentBadge}
          />
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          <Spotlight
            warm
            className="surface-card card-hover relative overflow-hidden p-6 md:col-span-2"
          >
            ...
            <div className="relative">
              ...
              <h2 className="mt-2 font-display text-2xl font-bold">
                {history.length > 0
                  ? `Try ${latestSkill} · ${latestType} next.`
                  ...
                  <div key={t} className="rounded-lg border border-border bg-white p-3">
                    ...
                    <div className="mt-1 font-display text-lg font-bold">
                      {stats.averageScore > 0
                        ? [
                            ...
                            Math.max(0, stats.averageScore - 2),
                    ...
                    <div className="mt-1 h-1 rounded-full bg-surface-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, (stats.averageScore > 0 ? [stats.averageScore + 4, Math.max(0, stats.averageScore - 2), stats.averageScore + 8][i] : 0) || 0)}%`,
                        }}
                      />
                    </div>
                  </div>
              ...
              <div className="mt-6">
                <MagneticButton id="start-recommended-btn" onClick={() => setCareerOpen(true)}>
                  <Sparkles className="h-4 w-4" /> Start recommended{" "}
...
function SparkChart({ data }: { data: number[] }) {
  if (data.length === 0) {
    return (
      <div className="py-8 text-center text-sm text-muted-foreground">
        Complete an assessment to see your trajectory.
      </div>
    );
  }
  const w = 640,
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="h-36 w-full">
      ...
      />
      {pts.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r="3" fill="#4b5563" />
      ))}
